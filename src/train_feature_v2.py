#!/usr/bin/env python3
"""LightGBM feature-v2: pseudo-abs magnitude, frequency encoding, feature diet."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/lgbm_feature_v2"),
...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Pseudo-absolute magnitude: safe for negative/near-zero redshift
    log_rz = np.log10(redshift_abs + 1e-4).astype("float32")
    for band in BANDS:
        df[f"{band}_abs_approx"] = (df[band].astype("float32") - 5.0 * log_rz).astype("float32")
